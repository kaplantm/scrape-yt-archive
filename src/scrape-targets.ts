...
export const scrapeTargets = async (
  ...
  const map = selectedTargets.map((target) => async () => {
    ...
    try {
      console.log(
        `////////// START check if already scraped  ${target.timestamp} //////////`
      );
      const result = await axios.get(
        `http://localhost:3000/api/video-scrape-instances?timestamp=${target.timestamp}`
      );
      const foundInstances = result.data;
      foundInstancesCount = foundInstances?.length;
      ...
      console.log(
        `////////// ${foundInstancesCount ? "FOUND" : "NOT FOUND"} ${
          target.timestamp
        } //////////`
      );
    } catch (e) {
      console.log(
        `////////// NOT FOUND - Error ${target.timestamp} //////////`
      );
    }
